...
<img src="assets/images/rendaoh_arch.png" width=800><br>
...
3. Communication Protocol (MQTT Topics)
  -  To connect to AWS IoT Core via MQTT, a client certificate is required.  Please access the AWS IoT Core service through the AWS Console and generate a client certificate.
  -  MicroPython's TLS implementation expects certificate files in DER format, not PEM.  Use the `openssl` command to convert your certificates from PEM to DER format, and upload them to the microcontroller's flash memory.
